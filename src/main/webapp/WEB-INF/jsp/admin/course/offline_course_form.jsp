...
<div class="title title_top">교육과정등록 - 집합교육 ${titleMode}<span></span></div>
...
<input type="hidden" name="mt_ctype_code" value="<%=SangsProperties.getProperty("Globals.mtCode_MT_CTYPE_CODE_OFFLINE")%>"/>
...
<input type="hidden" name="comp_val" value=""/> 
<input type="hidden" name="price" value="0"/> 
<input type="hidden" name="sample_yn" value=""/> 
<input type="hidden" name="previewurl" value=""/> 
...
<table class="table-type1 search_form">
    ...
    <tbody>
            ...
            <td>
                <select name="mt_sc_code" id="mt_sc_code" title="과정구분" style="width: 300px">
                    ...
                    <c:forEach var="emap" items="${course_title}" varStatus="status1">
                    	<c:if test="${emap.MT_SUB_CODE ne '4' }">
        ...
        <tr>
            <th>* 과정명</th>
            <td colspan="3"><input type="text"  class="line" name="coursetitle" id="coursetitle" value="${map.COURSETITLE}" style="width: 100%"/></td>
...
<script type="text/javascript">
    ...
    function writeFormCheck() {
        ...
        if(confirm( msg+ " 하시겠습니까?")) {
            ...
            var del_files = [];
            $('.del_file:checked').each(function(){
            	del_files.push($(this).val());
            });
            f1.del_files.value = del_files.join(',');
            f1.submit();
        }
	...
	function fnAdd() {
		...
		var str = '';
			...
			str = str + "<option value='<c:out value='${emap.EDU_LOAN_SEQ}'/>' <c:if test='${map.EDU_LOAN_SEQ == emap.EDU_LOAN_SEQ}'> selected='selected'</c:if>><c:out value='${emap.EDU_LOAN_NAME}'/></option>";
			str = str + "</c:forEach>";
			str = str + "</select>&nbsp;";
