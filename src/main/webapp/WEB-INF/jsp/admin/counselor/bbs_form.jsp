...
$(document).on('click', '#fnDel', function(){ 
    ...
    function go_exec(){
	    ...
	    //직종
	    if(form.mtJobCode.value == ''){
            alert('직종을 선택해 주세요.');
            return;
        }
    	  
	    //분야
	    if(form.mtCateCode.value == ''){
            alert('분야를 선택해 주세요.');
            return;
        }
	    
	    //상담상태
	    if(form.mtStateCode.value == ''){
            alert('상담상태를 선택해 주세요.');
            return;
        }
  ...
  //파일 삭제
			...
			success : function(data) {
				...
				if(data.length == 0){
					...
					$.each(data, function(i, item){
						if(i != 0){
							if(data[i].CMTNO != data[i-1].CMTNO){
								rowno += 1;
								rowcmtno = 1;
								btn = '<a href="#" onclick="cmt_reply_form(\'cmtrow_'+data[i].CMTNO+'_1\','+data[i].BBSNO+','+data[i].CMTNO+'); return false;" class="btn blue small">답글</a>';
	...
	function go_best_form(){
		$('form[name=pform]').attr('action', '/admin/counselor/bestBbsForm.do').submit();
	}
</script>
<!-- 본문 > 상단 > 타이틀 -->
...
<form name="pform" id="pform" action="/admin/site/bbsForm.do" method="post" enctype="multipart/form-data">
    ...
    <table class="table-type1 search_form">
      ...
      <tr>
          ...
          <th><em>*</em>공개여부</th>
          <td colspan="3">
 				<input type="radio" name="openAt" value="Y" <c:if test="${map.OPEN_AT eq 'Y'}"> checked="checked" </c:if> /> 공개 &nbsp;
				<input type="radio" name="openAt" value="N" <c:if test="${map.OPEN_AT eq 'N'}"> checked="checked" </c:if>checked /> 미공개         
          </td>
      ...
      <tr>
      	<th><em>*</em>분야</th>
        <td>
        	<select id="mtCateCode" name="mtCateCode" title="분야">
	  ...
	  <tr>
	            <th>상담사</th>
	            <td colspan="3">
	                <input type="text" name="counselorNm"  id="counselorNm" readonly="readonly" value="${map.AN_MBERNM}"  style="width:200px"   class="line"/>
             			<span id="compCounselor" <c:if test="${empty map.AN_USERNO}">style="display: none;"</c:if>>
						<a href="#" onclick="counselorSave('N'); return false;" class= "btn small red">삭제</a>
		            </span>
            			<span id="emptyCounselor" <c:if test="${not empty map.AN_USERNO}">style="display: none;"</c:if>>
		                <a href="#" onclick="popCounsel(); return false;" class= "btn small gray">검색</a>
		                <a href="#" onclick="counselorSave('Y'); return false;" class= "btn small blue">확정</a>
		            </span>
	                
	                <span id="counselorText" style="padding-left: 10px;font-weight: bold;color: blue;" >
                	<c:if test="${not empty map.AN_USERNO && map.AN_USERNO != ''}">상담사배정이 확정되었습니다.</c:if>
                	<%-- <c:when test="${not empty map.AN_USERNO}">상담 신청중입니다. 확정해주세요.</c:when> --%>
                	<c:if test="${empty map.AN_USERNO || map.AN_USERNO == ''}">등록된 상담사가 없습니다.</c:if>
	                </span> 
	            </td>
	        </tr>      
      </c:if>
      ...
      <tr>
      	<th><em>*</em>상담상태</th>
        <td>
        	<select id="mtStateCode" name="mtStateCode" title="분류">
			<option value="">선택</option>
			<c:forEach var="state" items="${stateData}"> 
				<option name="mtStateCode" value="${state.MT_SUB_CODE}" <c:if test="${state.MT_SUB_CODE eq map.COUNSEL_STATE_CODE}">selected="selected"</c:if>>${state.MT_SUB_NAME}</option>
			</c:forEach>
			</select>
        </td>
        <th><em>*</em>심화상당요청</th>
            <td>
   				<input type="radio" name="deepAt" value="Y" <c:if test="${map.DEEP_AT eq 'Y'}"> checked="checked" </c:if>/> 요청 &nbsp;
				<input type="radio" name="deepAt" value="N" <c:if test="${map.DEEP_AT eq 'N'}"> checked="checked" </c:if>checked /> 미요청
            </td>
      </tr>
      <tr>
      	  <th>신청자</th>
	      <td>
	         <input type="text" name="applyNm"  id="applyNm"  value="<c:out value="${userInfoMap.APPLYNM}"/>"  style="width:200px"   class="line"/>
	      </td>
	      <th>유선전화</th>
	      <td><input type="text" name="applyTel"  id="applyTel"  value="<c:out value="${userInfoMap.TELNO}"/>"  style="width:200px"  class="line"/></td>
	  </tr>
	  <tr>
	      <th>이메일</th>
	      <td>
	         <input type="text" name="applyEmail"  id="applyEmail"  value="<c:out value="${userInfoMap.EMAIL}"/>"  style="width:200px"   class="line"/>
	      </td>
	      <th> 휴대전화</th>
	      <td><input type="text" name="applyPhone"  id="applyPhone"  value="<c:out value="${userInfoMap.MOBLPHON}"/>"  style="width:200px"  class="line"/></td>
	  </tr>
      <tr>
         <th><em>*</em>내용</th>
    ...
    <div class="title title_top" <c:if test="${map.bbsNo eq null}">style="display: none;"</c:if>>답변내용</div>
    
    <table class="table-type1 search_form" <c:if test="${map.bbsNo eq null}">style="display: none;"</c:if>>
      <colgroup>
          <col class="cellc" width="15%">
          <col class="celll" width="35%">
          <col class="cellc" width="15%">
      	...
      	<th><em>*</em>작성자</th>
        <td>
        	<c:out value="${map.AN_MBERNM}"/>
        </td>
        <th><em>*</em>작성일</th>
        <td><c:out value="${map.AN_WATE}"/></td>
      </tr>
    	<tr>
    		<th>답변<br/>내용</th>
	         <td style="padding:20px 10px 20px 10px;" colspan="3">
...
<div class="button" >
	...
	<c:if test="${map.bbsNo ne null}">
		<a href="#" onclick="go_best_form(); return false;" class="btn green big">BEST 선정</a>
