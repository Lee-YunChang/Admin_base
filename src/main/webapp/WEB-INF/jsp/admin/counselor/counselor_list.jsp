...
function move_page(cPage){
	...
	var counselArr = new Array;
	$('input:checkbox[name=mtTutorCode]').each(function() {
		var ischecked = $(this).is(':checked');
		if(ischecked) {
			counselArr.push($(this).val());
		}
	});	 
	
	$("#fieldArr").val(fieldArr.join(','));
	$("#counselArr").val(counselArr.join(','));
	
	$('#cPage').val(cPage);
	$("#pform").attr("action", "/admin/counselor/counselorList.do").submit();
}
...
function list_page(){
    ...
    frm.target = "";
    frm.cPage.value = "1";
    $("#pform").attr("action", "/admin/counselor/counselorList.do");
    frm.submit();
}
...
$(document).ready(function(){
	...
	if($("#counselArr").val() == '') {
		...
		var counselArr = $("#counselArr").val().split(',');
		for (var nArrCnt in counselArr) {

	         $("input[name=mtTutorCode][value="+counselArr[nArrCnt]+"]").prop("checked",true);
...
<!-- 본문 > 상단 > 타이틀 -->
<div class="title title_top">상담사관리<span></span></div>
...
<input type="hidden" name="counselArr" id="counselArr" value="${REQUEST_DATA.counselArr}"/>
...
<table class="tb table-type1">
    ...
    <c:when test="${not empty resultList}">
	    ...
	    <tr align="center">
	        ...
	        <td><c:out value="${result.COUNSEL_DT}"/></td> 
	    </tr>
	    </c:forEach>
	</c:when>
	<c:otherwise>
