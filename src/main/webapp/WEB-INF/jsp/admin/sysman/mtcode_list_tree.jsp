...
<script type="text/javascript">
	...
	function requestAjax() {
		...
		$.ajax({
		        url : "/admin/common/mtCodeAjax.do",
				type : 'post',
				dataType : 'json',
	...
	function fn_detailAjax(event) {
		...
		$.ajax({
			url : "/admin/common/mtCodeDetailAjax.do",
	...
	function doDel() {
		...
		if(confirm("삭제후에는 복구가 불가능합니다. 해당 메뉴를 삭제 하시겠습니까?  ")) {
			...
			$.ajax({
		        url : "/admin/common/mtCodeSaveAjax.do",
		        data : {
	...
	function viewNewForm() {
		...
		/* if($("#cmmnCodeDpNo").val() >= 5) {
			alert("4뎁스 이상 메뉴를 생성할수 없습니다.");
			return;
		} */
		...
		$("#cmmnCodeDpNo").val(parseInt(cmmnCodeDpNo) + 1);
	}
							...
							<table class="row_type">
								...
								<tbody>
									...
									<tr>
										<th>코드번호</th>
										<td colspan="3">
											<input type="text" name="cmmnCodeSn" id="cmmnCodeSn" style="width: 25%" onKeyup="fn_defaultChk();"></input>
