...
<script type="text/javascript">
	...
	function fnAdd() {
		...
		var str = '';
		    ...
		    str = str + "<input type='file' id='uploadFile"+cnt+"' class='file_input_hidden' name='uploadFile"+cnt+"' >";
	...
	function tutorSubFormCheck(){
		...
		if(confirm(context)){
			...
			$("#insertForm").attr("mothod" , "post").attr("action" , "/admin/tutor/tutorCmmntySubInsert.do");
			form.submit();
		}
	};
...
<!-- 본문 > 상단 > 타이틀 -->
<div class="title title_top">커뮤니티코드관리 > 게시판 <c:if test="${UPDATE_DATA.NTTNO eq '' }">등록</c:if><c:if test="${UPDATE_DATA.NTTNO ne '' }">수정</c:if>
...
<form name="insertForm" id="insertForm" action="" method="post"  enctype="multipart/form-data">
			...
			<tr>
				<c:choose>
					...
					<c:otherwise>
						...
						<th>공지여부</th>
						<td>
							<input type="radio" name="noticeYn" id="noticeYn" value="Y" <c:if test="${UPDATE_DATA.NOTICE_AT eq 'Y' || UPDATE_DATA.NOTICE_AT eq null }">checked="checked"</c:if> />&nbsp공지 
							<input type="radio" name="noticeYn" id="noticeYn" value="N" <c:if test="${UPDATE_DATA.NOTICE_AT eq 'N' }">checked="checked"</c:if> />&nbsp공지 안함
						</td>
					</c:otherwise>
				</c:choose>	
			</tr>
			...
			<tr>
				...
				<td colspan="3" class="td-input-wrap">
					...
					<div  id="eduDate_${REQUEST_DATA.fileSCount+1}" class="input-wrap mb15">
					  	<input type="file" id="uploadFile${REQUEST_DATA.fileSCount+1}" name="uploadFile${REQUEST_DATA.fileSCount+1}" class='file_input_hidden' onchange="fnChange(${REQUEST_DATA.fileSCount+1}, 0);">
						<button class="scale-up" onclick="fnAdd();return false" title="추가">추가</button>
	...
	<div class="button">
		<c:if test="${REQUEST_DATA.pageMode eq pageModeReg}">
        	<a href="#" onclick="tutorSubFormCheck(); return false;" class= "btn big blue">등록</a>
         </c:if>
         <c:if test="${REQUEST_DATA.pageMode ne pageModeReg}">
             <a href="#" onclick="tutorSubFormCheck(); return false;" class= "btn big green">수정</a>
         </c:if>
