...
<script type="text/javascript">
	...
	function move_page(cPage){
		...
		$("form[name=pform]").attr("method", "post").attr("action", "/admin/user/nmberList.do").submit();
	}
	...
	function user_form(userNo){
		window.open("/admin/user/nmberForm.do?userNo="+userNo, "user_form", "width=800, height=500, scrollbars=yes");
	}
...
<!-- 본문 > 상단 > 타이틀 -->
<div class="title title_top">비회원정보관리<span></span></div>

	<!-- 본문 > 상단 > 검색영역 시작 -->
	<form name="pform" action="/admin/user/nmberList.do" method="post">
	...
	<table class="table-type1 search_form" summary="상상스토리 교육시스템 비회원 검색입니다.">
		<thead>
			<colgroup>
			  	<col style="width:8%">
			  	<col style="width:42%">
	...
	<table width="100%" class="btn_Group" summary="상상스토리 교육시스템 회원 목록입니다.">
		<tbody>
	  		<tr>
	   			<td class="pageinfo">조회건수 <span class="red"><fmt:formatNumber value="${resultList[0].TOTAL_CNT}" groupingUsed="true" /></span> 건</td></td>
	   			<td align="right">
					<a href="#" onclick="user_form(0);"class="btn sky">비회원등록</a>
 					<a href="#" onclick="smsFormGroup(document.userForm);" class="btn sky">SMS발송</a>
 					<a href="#" onclick="goExcel();" class="btn sky">엑셀다운로드</a>		
 					<a href="#" onclick="popup3('/admin/user/nmberListExcelForm.do',700,300,1,1);" class="btn sky">엑셀 업로드</a>						
 				</td>
	  		</tr>
	 	</tbody>
	</table>
		...
		<tbody>
			<c:forEach var="result" items="${resultList}" varStatus="status">
				<tr align=middle  onmouseover="onFocusBgCh(this);" class="onFocusBgCh">
					...
					<td><c:out value='${resultList[0].TOTAL_CNT - (REQUEST_DATA.cPage-1)*10 - status.count+1}'/></td> 
					<td><c:out value='${result.MT_GRADE_NAME}'/></td>
					<td><c:out value='${result.MT_OFFICE_NAME}'/></td>
					<td><a href="#" onclick="user_form('${result.USERNO}');"><b><c:out value='${result.MBERNM}'/></b></a></td>
					<td><c:out value='${result.SEX_NM}'/></td>
