...
<script type="text/javascript">
    ...
    function go_reply(){
        var form = document.form;
        form.qu.value = "reply";
		$("#pform").attr("action", "/admin/site/replyForm.do");
        form.pageMode.value = "<%=SangsProperties.getProperty("Globals.PAGE_MODE_RE_MOD")%>";
        form.submit();
    }
...
<form name="form" id="pform" action="/admin/site/bbsView.do" method="post" >
    ...
    <table class="table-type1 search_form" >
      ...
      <tr>      
          <th>분류</th>
	      <td><c:out value="${map.MT_CATE_CODE_NAME}"/></td>
	   </tr>
	   </c:if>
	   <c:if test="${map.ISTOP ==  'Y'}">
	   <tr>
	        <th>상위고정</th>
	        <td>
	           	<c:if test="${map.TOPYN eq 'Y'}">고정</c:if>
				<c:if test="${map.TOPYN eq 'N'}">고정안함</c:if>
			</td>
       </tr>
       </c:if>
       <tr>
