...
<script type="text/javascript">
	//수정
	function tutorSubFormCheck(depth){
		var form = document.insertForm;
		form.qu.value = "update";
		form.depth.value= depth;
		$("#insertForm").attr("mothod" , "post").attr("action" , "/admin/tutor/cmmntyInsertForm.do");
		form.submit();
	};
	...
	function tutorSubFormDelete(){
		...
		if(confirm("삭제하시겠습니까!")){
			...
			form.useAt.value = 'N';
			$("#insertForm").attr("action" , "/admin/tutor/cmmntySubInsert.do");
			form.submit();
		}
	}
	...
	function subInfoReply(){
		...
		$("#insertForm").attr("action" , "/admin/tutor/cmmntyInsertForm.do");
		form.submit();
	}
...
<!-- 본문 > 상단 > 타이틀 -->
<div class="title title_top">커뮤니티관리 - 게시판 상세<span></span></div>
...
<form name="insertForm" id="insertForm" action="" method="post">
	...
	<div class="button">
        ...
        <c:if test="${REQUEST_DATA.pageMode ne pageModeReg && REQUEST_DATA.registId eq REQUEST_DATA.SES_USERID && REQUEST_DATA.useAt eq 'Y'}" >
            <a href="#" onclick="tutorSubFormCheck('<c:out value="${VIEW.NTT_DEPTH}" />'); return false;" class= "btn big green">수정</a>
