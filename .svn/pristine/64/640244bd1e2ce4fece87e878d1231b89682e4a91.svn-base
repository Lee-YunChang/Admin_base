<%
/****************************************************
    system	: 시스템관리 > 로그관리 > 개인정보처리로그
    title	: 개인정보처리로그 리스트 및 엑셀 다운
    summary	:
    wdate	: 2013-06-26
    writer	: sangs
*****************************************************/
...
<script type="text/javascript">
    ...
    function move_page(cPage){
        $("#listType").val("");
        $("#cPage").val(cPage);
        $("#pform").attr("action", '/admin/sysman/admlogTransList.do');
        $("#pform").submit();
    }
...
<c:set var="mbrTransGbSelect" value='<%=SangsProperties.getProperty("Globals.MBR_TRANS_GB_SELECT") %>' />
<c:set var="mbrTransGbCreate" value='<%=SangsProperties.getProperty("Globals.MBR_TRANS_GB_CREATE") %>' />
<c:set var="mbrTransGbUpdate" value='<%=SangsProperties.getProperty("Globals.MBR_TRANS_GB_UPDATE") %>' />
<c:set var="mbrTransGbDelete" value='<%=SangsProperties.getProperty("Globals.MBR_TRANS_GB_DELETE") %>' />
<c:set var="mbrTransGbPrint" value='<%=SangsProperties.getProperty("Globals.MBR_TRANS_GB_PRINT") %>' />
...
<!-- 본문 > 상단 > 타이틀 -->
<div class="title title_top">개인정보처리로그<span></span></div>
      ...
      <colgroup>
        ...
        <tr>
            ...
            <td>
                ...
                <select name="transGb" id="transGb" title="처리구분옵션">
                    <option value="">처리구분 전체</option>
                    <option value="<c:out value="${mbrTransGbSelect}"/>" <c:if test="${REQUEST_DATA.transGb eq mbrTransGbSelect}">selected</c:if>>조회</option>
                    <option value="<c:out value="${mbrTransGbCreate}"/>" <c:if test="${REQUEST_DATA.transGb eq mbrTransGbCreate}">selected</c:if>>등록</option>
                    <option value="<c:out value="${mbrTransGbUpdate}"/>" <c:if test="${REQUEST_DATA.transGb eq mbrTransGbUpdate}">selected</c:if>>수정</option>
                    <option value="<c:out value="${mbrTransGbDelete}"/>" <c:if test="${REQUEST_DATA.transGb eq mbrTransGbDelete}">selected</c:if>>삭제</option>
                    <option value="<c:out value="${mbrTransGbPrint}"/>" <c:if test="${REQUEST_DATA.transGb eq mbrTransGbPrint}">selected</c:if>>출력</option>
                </select>
            </td>
        </tr>
          <tr>
            ...
            <td colspan="3">
                 <select name="searchMode" id="searchMode" title="검색옵션">
                        ...
                        <option value="TRANS_USERID" <c:if test="${REQUEST_DATA.searchMode eq 'TRANS_USERID'}">selected</c:if>>아이디</option>
                        <option value="TRANS_IP" <c:if test="${REQUEST_DATA.searchMode eq 'TRANS_IP'}">selected</c:if>>IP주소</option>
                </select>
