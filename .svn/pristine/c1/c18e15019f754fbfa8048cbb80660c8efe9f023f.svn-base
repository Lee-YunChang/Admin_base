...
<script type="text/javascript">
	...
	function list_page(){
		...
		form.cPage.value = '1';
		$("form[name=pform]").attr("action" , "/admin/tutor/cmmntyList.do").submit();
	}
	
	/* 게시판내용 */
	function moveCmmnty(bbsNo , bbsAnswerAt, bbsNoticeAt){
		...
		pform.searchMode.selectedIndex = '0'; //셀렉트박스 초기화
		$("form[name=pform]").attr("action" , "/admin/tutor/cmmntyList.do").submit();
	...
	function viewInfo(nttNo , bbsNo, registId, useAt ){
		...
		$("form[name=pform]").attr("action" , "/admin/tutor/cmmntyViewForm.do").submit();
	}
	...
	function inserSubTutorCmmnty(bbsNo , bbsAnswerAt , qu){
		...
		pform.qu.value = qu;
		$("form[name=pform]").attr("action" , "/admin/tutor/cmmntyInsertForm.do").submit();
	}

	//페이징
	function move_page(cPage){
		var frm = document.pform;
		$("#pform").attr("action", "/admin/tutor/cmmntyList.do");
...
<!-- 본문 > 상단 > 타이틀 -->
<div class="title title_top">커뮤니티관리 <span></span></div>
<table width=100% border=0 cellspacing=0 cellpadding=0 class="exp-tab">
	<tbody>
		<tr>
			<c:forEach var="result" items="${LIST_DATA}">
			<td class="<c:choose><c:when test="${REQUEST_DATA.bbsNo == result.bbsNo}">on</c:when><c:otherwise>off</c:otherwise></c:choose>">
				<a href="#" onclick="moveCmmnty('<c:out value="${result.bbsNo}"/>' , '<c:out value="${result.bbsNoticeAt}"/>' , '<c:out value="${result.bbsAnswerAt}"/>'); return false"><c:out value="${result.bbsName}" /></a>
			</td>
			</c:forEach>
		</tr>
	</tbody>
...
<table width="100%" class="btn_Group">  <tbody>
    <tr>
		...
		<td align="right">
			<a href="#" onclick="inserSubTutorCmmnty('<c:out value="${REQUEST_DATA.bbsNo}" />' , '<c:out value="${REQUEST_DATA.bbsAnswerAt}" />' , 'insert');" class="btn blue" >등록</a>
		</td>
    </tr>
  </tbody>
</table>
