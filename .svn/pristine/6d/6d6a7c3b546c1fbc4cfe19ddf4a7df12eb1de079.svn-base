...
$(document).on('click', '#fnDel', function(){ 
    ...
    function go_list(){
		...
		$("#pform").attr("action", "/admin/tutor/bbsList.do");
        form.submit();
    }
    ...
    function go_exec(){
		...
		if(confirm("저장하시겠습니까?")) {
			...
			$("#pform").attr("action", "/admin/tutor/bbsExec.do");
	        form.submit();
		}
    } 
	...
	function delBtn(fileId , cnt , savePath , saveFile,bbsno){ 
		...
		if(fileId != ''){
			if(confirm("기존파일을 삭제하시겠습니까?")){
		        $.ajax({
		            url : "/admin/tutor/deleteEdBbsFileAjax.do",
		            data : {fileId : fileId, type : 'D' , savePath : savePath , saveFile : saveFile, bbsno : bbsno},
		            dataType : "json",
		            type : 'post',
		            success : function(res) { 
		            	if(res == "SUCCESS"){
