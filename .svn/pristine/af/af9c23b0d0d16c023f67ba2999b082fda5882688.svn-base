...
<script type="text/javascript">
	...
	function write_page(){
		var frm = document.pform;
		frm.qu.value = 'insert';
		$("#pform").attr("action", "/admin/site/bbsForm.do");
		frm.submit();
	}
...
<table class="tb table-type1">
	...
	<tr align=middle  onmouseover="onFocusBgCh(this);" class="onFocusBgCh">
		...
		<c:if test="${result.TOPYN ne 'Y' }"><td><c:out value="${result.RNUM}"/></td></c:if>
		<c:if test="${result.BCATENO eq  2}">
			<td><c:out value="${result.MT_CATE_CODE_NAME}"/></td>
		</c:if>
	  	<td style="text-align:left;padding-left:15px">
  		<c:if test="${result.REFSTEP > 0}">
        	<c:set var="refStep">${result.REFSTEP}</c:set>
        	<c:forEach var="x" begin="0" end="${refStep}" varStatus="status2">
        		<c:choose><c:when test="${x == (refStep-1)}">&nbsp;&nbsp;</c:when><c:otherwise>&nbsp;&nbsp;</c:otherwise></c:choose>
        	</c:forEach>
        </c:if>
