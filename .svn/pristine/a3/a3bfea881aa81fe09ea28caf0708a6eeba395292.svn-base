...
function fndelete(tno){ 
	if(confirm('삭제하시겠습니까?')){
		...
		$("#listform").attr("action", "/admin/sysman/smsDeleteExec.do");
		form.submit();	
	}
}
	//수정/ 등록 이동
	function goEdit(tno, q){   
		var tplno=tno;
		var qu=q;
		
		var frm = document.listform;
		window.open('','smstplInfo','width=470,height=600');
		frm.target = 'smstplInfo';
	 	 
		frm.qu.value = qu;
		frm.tplno.value = tplno;
		if(qu == 'insert') {
...
<!-- 본문 > 상단 > 타이틀 -->
<div class="title title_top">SMS탬플릿 관리<span></span></div>
...
<!-- 본문 > 메인 > 목록 시작  -->
<form id="listform" name="listform" action="smsmanTplInfo.do" method="post"  >
...
<table width="100%" class="btn_Group">
  <tbody>
	<tr>
		...
		<td align=right> 
			<a href="#" onclick="goEdit( '0' , 'insert' );" class="btn blue">등록</a>
  	</td>
	</tr>
  </tbody>
</table>
	...
	<c:if test="${fn:length(LIST_DATA) > 0}">
    	<c:forEach var="result" items="${LIST_DATA}" varStatus="status">
			<tr align=middle  onmouseover="onFocusBgCh(this);" class="onFocusBgCh">
				...
				<td align="left"><a href="#" onclick="goEdit('<c:out value="${result.TPLNO}" />' , 'update');"><c:out value="${result.TPLNAME}" /></a> </td>
				<td><c:out value="${result.TPL_UID}" /></td>
				<td><c:out value="${result.USEYN}" /></td>
				<td>
					<a href="#" onclick="goEdit('<c:out value="${result.TPLNO}" />' , 'update');" class="btn green small">수정</a>
