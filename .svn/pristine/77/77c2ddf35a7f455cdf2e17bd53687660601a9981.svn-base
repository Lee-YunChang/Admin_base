...
<div id="content">
	...
	<div class="round-box-small blue">
		...
		<input type="hidden" name="qu" id="qu" value="<c:out value="N"/>"/>
			...
			<table summary="상상스토리 교육시스템 클래스데스크의 문제등록입니다.">
				...
				<tbody>
					...
					<tr>
						<th>문제이미지</th>
						<td>
							<label for="question-img">과제명</label><input type="file" class="w70" name="qsubjectFile" id="qsubjectFile" />
								...
								<ul>
									<li>
										...
										<blockquote>
											...
											<input type="radio" id="rcode" name="rcode" value="1" checked="checked" <c:if test="${rowData.RCODE eq '1'}"> checked</c:if>> <label for="rcode" class="visible" >정답</label>
					...
					<tr>
						<th>작성자</th> 
						<td><input type="text" style="width: 80%" name="writer" id="writer"  maxlength="16" value="<c:out value="${rowData.WRITER}" default="${REQUEST_DATA.SES_USERNAME}"/>" readonly="readonly" > </td>
					</tr>
					<tr>
						<th>사용여부</th>
						<td>
							...
							<input type="radio" class="mid"   id="use-no" name="useyn" value="N" <c:if test="${rowData.USEYN eq 'N'}"> checked</c:if>><label for="use-no" class="visible">사용안함</label>
						</td>
					</tr>
				</tbody>
			</table>
...
<script type="text/javascript">
	...
	function fnSave(){
		...
		var qu = '<c:out value="${REQUEST_DATA.qu}"/>' ;
		$('#qu').val(qu);
